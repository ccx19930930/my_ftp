...
void server_cd(int sfd)
{
	...
	if(!strcmp(d.buf,"-"))
	{
		memset(&pwd,0,sizeof(pwd));
		memset(&pwd_id,0,sizeof(pwd_id));
		strcpy(pwd,u[sfd].lastpwd);
		strcpy(pwd_id,u[sfd].lastid);
		goto lable;
	}
	if(d.buf[0]=='~')
	{
...
lable:
	temp=1;
	send(sfd,(void*)&temp,sizeof(temp),0);
	memset(u[sfd].lastpwd,0,sizeof(u[sfd].lastpwd));
	memset(u[sfd].lastid,0,sizeof(u[sfd].lastid));
	strcpy(u[sfd].lastpwd,u[sfd].pwd);
	strcpy(u[sfd].lastid,u[sfd].pwd_id);
